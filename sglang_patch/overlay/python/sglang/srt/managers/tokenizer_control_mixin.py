...
BASE_LORA_ROUTER_NAME = "base_model"
...
_COMMUNICATOR_SPECS = [
    ...
    ("configure_lora_router", ConfigureLoRARouterReqOutput),
    ("switch_lora_adapter", SwitchLoRAAdapterReqOutput),
...
class TokenizerControlMixin:
    ...
    async def configure_lora_router(
        self: TokenizerManager,
        ...
        if not self.server_args.enable_lora:
            return ConfigureLoRARouterReqOutput(
                success=False,
                message="LoRA is not enabled. Please set `--enable-lora`.",
            )

        prev_ids = list(getattr(self, "_lora_router_hold_ids", []))
        try:
            acquired_ids: List[str] = []
            if obj.switch_every_n_tokens <= 0 and obj.mode not in (
                "prefill_decode",
                ...
                if obj.mode not in (
                    ...
                    raise ValueError(f"Unsupported LoRA router mode: {obj.mode}")
                if obj.mode in (
                    "prefill_decode",
                    "prefill_decode_ba_interval",
                ) and len(obj.lora_pool) != 2:
                    raise ValueError(f"{obj.mode} requires exactly two LoRA adapters")
                if obj.mode in ("route_decode", "route_decode_v2") and len(obj.lora_pool) < 1:
                    raise ValueError(f"{obj.mode} requires at least one LoRA adapter")
                if obj.mode == "token_interval" and len(obj.lora_pool) < 1:
                    raise ValueError("token_interval requires at least one LoRA adapter")
                lora_ids = []
                for lora_name in obj.lora_pool:
                    if lora_name == BASE_LORA_ROUTER_NAME:
                        lora_ids.append(None)
                    else:
                        lora_id = await self.lora_registry.acquire(lora_name)
                        lora_ids.append(lora_id)
                        acquired_ids.append(lora_id)
                obj.lora_ids = lora_ids

            results = await self.configure_lora_router_communicator(obj)
            success, message = FanOutCommunicator.merge_results(results)
            if success:
                if prev_ids:
                    await self.lora_registry.release(prev_ids)
                self._lora_router_hold_ids = [
                    lora_id for lora_id in (obj.lora_ids or []) if lora_id is not None
                ]
                return ConfigureLoRARouterReqOutput(
                    success=True,
                    message=message,
                    mode=obj.mode,
                    lora_pool=list(obj.lora_pool),
                    switch_every_n_tokens=obj.switch_every_n_tokens,
                )

            if acquired_ids:
                await self.lora_registry.release(acquired_ids)
            return ConfigureLoRARouterReqOutput(success=False, message=message)
        except Exception as e:
            if "acquired_ids" in locals() and acquired_ids:
                await self.lora_registry.release(acquired_ids)
            return ConfigureLoRARouterReqOutput(success=False, message=str(e))

    async def switch_lora_adapter(
        self: TokenizerManager,
        obj: SwitchLoRAAdapterReqInput,
        _: Optional[fastapi.Request] = None,
    ) -> SwitchLoRAAdapterReqOutput:
        self.auto_create_handle_loop()
        ...
        try:
            ...
            results = await self.switch_lora_adapter_communicator(obj)
            success, message = FanOutCommunicator.merge_results(results)
            if not success:
                await self.lora_registry.release(obj.lora_id)
            else:
                holds = getattr(self, "_lora_switch_hold_ids_by_rid", None)
                if holds is None:
                    holds = {}
                    self._lora_switch_hold_ids_by_rid = holds
                holds.setdefault(obj.request_id, []).append(obj.lora_id)
            return SwitchLoRAAdapterReqOutput(success=success, message=message)
        except Exception as e:
            if obj.lora_id:
                await self.lora_registry.release(obj.lora_id)
