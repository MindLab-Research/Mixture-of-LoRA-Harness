...
from sglang.srt.managers.io_struct import (
    ...
    ConfigureLoRARouterReqInput,
    ConfigureLoRARouterReqOutput,
    ...
    SlowDownReqInput,
    SlowDownReqOutput,
    SwitchLoRAAdapterReqInput,
    SwitchLoRAAdapterReqOutput,
...
class Scheduler(
    ...
    def __init__(
        ...
        self.lora_router_pool: List[Optional[str]] = []
        self.lora_router_names: List[str] = []
    ...
    def init_request_dispatcher(self):
        self._request_dispatcher = TypeBasedDispatcher(
            [
                ...
                (ConfigureLoRARouterReqInput, self.configure_lora_router),
                (SwitchLoRAAdapterReqInput, self.switch_lora_adapter),
                (
                    GetLoadsReqInput,
    ...
    def _get_route_decode_params(self, req: Req) -> Dict[str, Any]:
        ...
        if not isinstance(params, dict):
            return {}
        if params.get("mode") not in ("route_decode", "route_decode_v2"):
            return {}
        return params
    ...
    def _route_decode_signal_override(
        ...
        route_to_adapter = params.get("route_to_adapter") or {}
        if not isinstance(user_text, str) or not isinstance(route_signals, dict):
            return None
        if not isinstance(route_to_adapter, dict):
            route_to_adapter = {}
    ...
    def _maybe_route_lora_route_decode(self, batch: ScheduleBatch) -> None:
        if self.lora_router_mode not in ("route_decode", "route_decode_v2"):
            return
        for index, req in enumerate(batch.reqs):
            ...
            seq_len = self._apply_route_decode_selection(req, route_id, params, decision)
    ...
    def _maybe_route_lora_after_prefill(self, batch: ScheduleBatch) -> None:
        if self.lora_router_mode in ("route_decode", "route_decode_v2"):
    ...
    def _get_new_batch_prefill_raw(
        ...
        if self.enable_lora:
            ...
            if self._lora_router_enabled() and self.lora_router_mode not in (
                "route_decode",
                "route_decode_v2",
            ):
                running_loras.update(self.lora_router_pool)
    ...
    def configure_lora_router(
        self, recv_req: ConfigureLoRARouterReqInput
    ) -> ConfigureLoRARouterReqOutput:
        """Configure request-local LoRA switching while keeping live KV state."""

        try:
            lora_ids = list(recv_req.lora_ids or [])
            if recv_req.switch_every_n_tokens <= 0 and recv_req.mode not in (
                "prefill_decode",
                "prefill_decode_ba_interval",
                "route_decode",
                "route_decode_v2",
                ...
                self.lora_router_switch_every_n_tokens = 0
                self.lora_router_mode = "token_interval"
                self.lora_router_seed = recv_req.seed
                return ConfigureLoRARouterReqOutput(
                    success=True,
                    message="LoRA router disabled.",
                )

            if not self.enable_lora:
                raise ValueError("LoRA is not enabled.")
            if recv_req.mode not in (
                "token_interval",
                "prefill_decode",
                "prefill_decode_ba_interval",
                "route_decode",
            ...
            if recv_req.mode in ("route_decode", "route_decode_v2") and len(lora_ids) < 1:
            ...
            if recv_req.mode in ("route_decode", "route_decode_v2"):
                unsupported_loras = [
                    lora_id
                    for lora_id in dict.fromkeys(lora_ids)
                    if not self.tp_worker.model_runner.lora_manager.validate_lora_batch(
                        {lora_id, None}
                    )
                ]
                if unsupported_loras:
                    raise ValueError(
                        "One or more route_decode LoRA adapters cannot fit with "
                        f"the base model in the LoRA memory pool: {unsupported_loras}"
                    )
            else:
                unique_lora_ids = set(lora_ids + [None])
                if len(unique_lora_ids) > self.max_loras_per_batch:
                    raise ValueError(
            ...
            self.lora_router_seed = recv_req.seed
            logger.info(
                "Configured LoRA KV reuse router: mode=%s lora_pool=%s lora_ids=%s "
                "switch_every_n_tokens=%d",
                self.lora_router_mode,
                self.lora_router_names,
                self.lora_router_pool,
                self.lora_router_switch_every_n_tokens,
            )
            return ConfigureLoRARouterReqOutput(
                success=True,
                message="LoRA router configured.",
                mode=self.lora_router_mode,
                lora_pool=self.lora_router_names,
                switch_every_n_tokens=self.lora_router_switch_every_n_tokens,
            )
        except Exception as e:
            logger.exception("Failed to configure LoRA router")
            return ConfigureLoRARouterReqOutput(success=False, message=str(e))

    def switch_lora_adapter(
        self, recv_req: SwitchLoRAAdapterReqInput
    ) -> SwitchLoRAAdapterReqOutput:
        target_id = recv_req.lora_id
        for req in list(self.running_batch.reqs) + list(self.waiting_queue):
            if req.rid == recv_req.request_id or req.rid.startswith(
                recv_req.request_id
            ):
                if target_id is not None:
                    if not self.tp_worker.model_runner.lora_manager.validate_lora_batch(
                        ...
                        return SwitchLoRAAdapterReqOutput(
                            success=False,
                            message="Target LoRA cannot fit in the LoRA memory pool.",
                        )
                self._switch_req_lora(req, target_id, "manual")
                return SwitchLoRAAdapterReqOutput(
                    success=True,
                    message=(
                        f"Request {recv_req.request_id} switched to "
                        f"LoRA {recv_req.lora_name}."
                    ),
                )

        return SwitchLoRAAdapterReqOutput(
            success=False,
            message=f"Request {recv_req.request_id} was not found.",
        )
