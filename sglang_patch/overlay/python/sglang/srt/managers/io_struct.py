...
class ConfigureLoRARouterReqInput(BaseReq):
    # User-facing LoRA adapter names. TokenizerManager resolves these to lora_ids
    # before forwarding to scheduler processes.
    lora_pool: List[str]
    switch_every_n_tokens: int = 0
    seed: int = 0
    mode: str = "token_interval"
    # Filled by TokenizerManager; the scheduler only consumes stable lora_ids.
