...
DEFAULT_LORA_LIBRARY_DIR = os.getenv("SGLANG_LORA_LIBRARY_DEFAULT", "lora_library")
LORA_LIBRARY_DIR = Path(os.getenv("SGLANG_LORA_LIBRARY_DIR", DEFAULT_LORA_LIBRARY_DIR))
LORA_ENTRY_ROUTE_ID = os.getenv("SGLANG_LORA_ENTRY_ROUTE_ID", "L0")
...
def _parse_lora_library_markdown(path: Path) -> Dict[str, Any]:
...
def _load_lora_router_library(library_dir: Path) -> Dict[str, Any]:
    ...
    for path in sorted(library_dir.glob("*.md")):
        item = _parse_lora_library_markdown(path)
        if item["dataset"] and item["dataset"] not in item["datasets"]:
            item["datasets"].insert(0, item["dataset"])
        tasks[item["id"]] = item
        if item["id"] == LORA_ENTRY_ROUTE_ID:
            entry_metadata = item

    if entry_metadata is None:
        raise FileNotFoundError(f"Missing {LORA_ENTRY_ROUTE_ID}.md in {library_dir}")
    if not tasks:
        raise FileNotFoundError(f"No LoRA markdown files found in {library_dir}")

    route_to_adapter = {
        task_id: str(task.get("adapter_name") or task_id)
        for task_id, task in tasks.items()
        if task.get("adapter_name")
    }
    return {
        "success": True,
        "library_dir": str(library_dir),
        "entry_route_id": LORA_ENTRY_ROUTE_ID,
        "base_route_id": LORA_ENTRY_ROUTE_ID,
        "base_route_adapter": str(entry_metadata.get("adapter_name") or LORA_ENTRY_ROUTE_ID),
        "tasks": tasks,
        "entry_metadata": entry_metadata,
        "base_model_metadata": entry_metadata,
...
@app.api_route("/v1/configure_lora_router", methods=["POST"])
@auth_level(AuthLevel.ADMIN_OPTIONAL)
async def configure_lora_router(obj: ConfigureLoRARouterReqInput, request: Request):
    """Configure request-local LoRA switching while preserving live KV cache."""
    result = await _global_state.tokenizer_manager.configure_lora_router(obj, request)
    return ORJSONResponse(
        result,
...
@app.api_route("/v1/lora_router_library", methods=["GET"])
@app.api_route("/v1/lora-router-library", methods=["GET"])
async def get_lora_router_library():
    """Return server-local LoRA Library metadata for remote clients."""
    try:
        return ORJSONResponse(
            _load_lora_router_library(LORA_LIBRARY_DIR),
            status_code=HTTPStatus.OK,
        )
    except Exception as e:
        return ORJSONResponse(
            {
                "success": False,
                "library_dir": str(LORA_LIBRARY_DIR),
                "message": str(e),
            },
...
@app.api_route("/v1/switch_lora_adapter", methods=["POST"])
@auth_level(AuthLevel.ADMIN_OPTIONAL)
async def switch_lora_adapter(obj: SwitchLoRAAdapterReqInput, request: Request):
    """Switch an in-flight request to a different LoRA adapter."""
    result = await _global_state.tokenizer_manager.switch_lora_adapter(obj, request)
    return ORJSONResponse(
        result,
        status_code=HTTPStatus.OK if result.success else HTTPStatus.BAD_REQUEST,
    )
